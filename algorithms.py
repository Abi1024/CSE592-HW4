...
def subgradient_descent( func, initial_x, maximum_iterations=65536, initial_stepsize=1):
    ...
    while True:
        ...
        update_step = initial_stepsize/(np.sqrt(iterations+1))
        x = x - update_step*gradient;
...
def adagrad( func, initial_x, maximum_iterations=65536, initial_stepsize=1, initial_sum_of_squares=1e-3):
    ...
    start_time = time.time()
    iterations = 0
    gradients = np.zeros((maximum_iterations,initial_x.shape[0]))
    # subgradient updates
    while True:
        value, gradient = func( x , 1 )
        value = np.double( value )
        ...
        runtimes.append( time.time() - start_time )
        xs.append( x.copy() )
        gradients[iterations] = gradient.T
        if (iterations % 1000 == 0):
            print(iterations)
        # x = ( TODO: update of adagrad )
        update_step = initial_stepsize/(initial_sum_of_squares + np.sum(np.square((gradients[0:iterations,:])),axis = 0) )
        x -= update_step*gradient
